...
// well under the paid-tier RPM while making bulk runs ~Nx faster.
...
// Re-run the arithmetic reconciliation over every stored bill and reload.
async function runValidation() {
    ...
    try {
        ...
        if (data.success) {
            const p = data.summary.purchase || {};
            const s = data.summary.sales || {};
            showToast(`Validated ${p.total || 0} purchase (${p.review || 0} flagged), ${s.total || 0} sales (${s.review || 0} flagged)`, 'success');
...
// Single-bill reprocess: preview the re-extraction diff, then let the user apply.
async function reprocessBill(billId) {
    ...
    try {
        const res = await fetch(`/api/bills/reprocess/${billId}`, {
            method: 'POST',
...
// Bulk: re-extract every flagged bill (bounded pool) and auto-apply the clean ones.
async function bulkReprocessFlagged() {
    ...
    async function worker() {
        while (true) {
            ...
            const bill = queue[i];
            try {
                const res = await fetch(`/api/bills/reprocess/${bill.id}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
...
// Bills currently visible given the flagged-only toggle and the search box.
...
async function processFiles() {
    ...
    // Bounded concurrency pool: several files extract at once instead of
    // waiting for each Gemini round-trip in turn. Wall-clock ≈ the slowest
    // file, not the sum. Capped well under the paid-tier RPM, leaving room
    // for any A2 re-extraction calls.
